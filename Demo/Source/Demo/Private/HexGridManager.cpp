...
static void DumpNeighborsOf(const UHexGridManager* Grid, const FHexAxialCoordinates& C, const TCHAR* Label)
{
    ...
    if (Grid)
    {
        ...
        FString S = FString::JoinBy(Out, TEXT(" "),
            [](const FHexAxialCoordinates& X){ return FString::Printf(TEXT("(%d,%d)"), X.Q, X.R); });
...
void UHexGridManager::RebuildGrid()
{
    ...
    for (auto& Kvp : TilesMap)
        if (AHexTile* T = Kvp.Value) T->Destroy();
    TilesMap.Empty();
...
FVector UHexGridManager::ComputeTileSpawnPosition(int32 Q, int32 R) const
{
    ...
    if (bHit) FinalZ = Hit.Location.Z + TileZOffset;
...
bool UHexGridManager::TryComputeTileSpawnPosition(int32 Q, int32 R, FVector& OutLocation) const
{
    ...
    {
        if (!HitA) { UE_LOG(LogTemp, Warning, TEXT("[HexGrid] (%d,%d): hit no actor -> skip"), Q, R); return false; }
...
{
    if (AHexTile* const* Found = TilesMap.Find(Coords)) return *Found;
    return nullptr;
}

TArray<FHexAxialCoordinates> UHexGridManager::GetNeighbors(const FHexAxialCoordinates& C) const
{
    TArray<FHexAxialCoordinates> Out; Out.Reserve(6);
    for (int i = 0; i < 6; ++i)
    {
        const FHexAxialCoordinates N{ C.Q + GDQ6[i].Q, C.R + GDQ6[i].R };
        if (GetHexTileAt(N)) Out.Add(N);
...
void UHexGridManager::BuildWorldNeighbors()
{
    ...
    for (const auto& ItA : Pos)
    {
        ...
        struct FNeighborDist { FHexAxialCoordinates Key; float D2; };
        TArray<FNeighborDist> D; D.Reserve(16);

        for (const auto& ItB : Pos)
        {
            if (ItB.Key == AKey) continue;
            const float d2 = (ItB.Value - APos).SizeSquared2D(); // float sûr
            D.Add({ ItB.Key, d2 });
        }

        D.Sort([](const FNeighborDist& L, const FNeighborDist& R){ return L.D2 < R.D2; });

        TArray<FHexAxialCoordinates> Neigh;
        const int32 Count = FMath::Min(6, D.Num());
        Neigh.Reserve(Count);
        for (int32 i = 0; i < Count; ++i) Neigh.Add(D[i].Key);
...
{
    if (const TArray<FHexAxialCoordinates>* Found = WorldNeighbors.Find(From)) Out = *Found;
    else Out.Reset();
}
